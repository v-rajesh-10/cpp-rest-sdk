...
public:
  ...
  Response& addField(const std::string& name, const std::string& value);
  const std::vector<uint8_t>& getBody() const;
  Response& setBody(const std::vector<uint8_t>& body);
  void fromString(const std::string& string);
  std::string toString() const;
  friend std::istream& operator>>(std::istream& in, Response& request);
  friend std::ostream& operator<<(std::ostream& out, const Response& request);
